...
class MemoryService {
    ...
    private val invariantsFile: File
        get() = File(memoryDir, "invariants.md")
...
""".trimIndent()
    ...
    /**
     * Read invariants from file. Creates default if not exists.
     */
    fun readInvariants(): List<Invariant> {
        if (!invariantsFile.exists()) {
            createDefaultInvariants()
        }
        return try {
            parseInvariants(invariantsFile.readText())
        } catch (e: Exception) {
            println("Error reading invariants: ${e.message}")
            emptyList()
        }
    }

    /**
     * Add auto-generated invariant
     */
    fun addAutoInvariant(text: String) {
        val invariants = readInvariants().toMutableList()
        val normalizedText = text.trim()

        // Check if already exists
        if (invariants.any { it.text.equals(normalizedText, ignoreCase = true) }) {
            return
        }

        invariants.add(Invariant(normalizedText, isAutoGenerated = true))
        saveInvariants(invariants)
        println("┃  📌 Added invariant: $normalizedText")
    }

    /**
     * Remove invariant by text
     */
    fun removeInvariant(text: String) {
        val invariants = readInvariants().toMutableList()
        val removed = invariants.removeAll { it.text.equals(text.trim(), ignoreCase = true) }
        if (removed) {
            saveInvariants(invariants)
            println("┃  🗑 Removed invariant: $text")
        }
    }
    ...
    private fun saveInvariants(invariants: List<Invariant>) {
        val content = buildString {
            ...
            appendLine()
            appendLine("Правила и ограничения, которые ВСЕГДА должны соблюдаться.")
            appendLine()

            val userInvariants = invariants.filter { !it.isAutoGenerated }
            val autoInvariants = invariants.filter { it.isAutoGenerated }

            appendLine("## Пользовательские инварианты")
            appendLine("<!-- Добавляйте свои инварианты ниже в формате: - Текст инварианта -->")
            appendLine()
            if (userInvariants.isEmpty()) {
                appendLine("_Нет пользовательских инвариантов_")
            } else {
                userInvariants.forEach { appendLine("- ${it.text}") }
            }
            appendLine()

            appendLine("## Автоматически извлечённые")
            appendLine("<!-- Эти инварианты извлекаются автоматически из диалога -->")
            appendLine()
            if (autoInvariants.isEmpty()) {
                appendLine("_Нет автоматических инвариантов_")
            } else {
                autoInvariants.forEach { appendLine("- ${it.text}") }
            }
        }

        try {
            invariantsFile.writeText(content)
        } catch (e: Exception) {
            println("Error saving invariants: ${e.message}")
        }
    }

    private fun parseInvariants(content: String): List<Invariant> {
        val invariants = mutableListOf<Invariant>()
        var currentSection = ""

        content.lines().forEach { line ->
            when {
                line.startsWith("## Пользовательские") -> currentSection = "user"
                line.startsWith("## Автоматически") -> currentSection = "auto"
                line.startsWith("- ") && !line.contains("_Нет") -> {
                    val text = line.removePrefix("- ").trim()
                    if (text.isNotEmpty()) {
                        invariants.add(Invariant(text, isAutoGenerated = currentSection == "auto"))
                    }
                }
            }
        }
    ...
    private fun createDefaultInvariants() {
        val defaultContent = """
# Инварианты

Правила и ограничения, которые ВСЕГДА должны соблюдаться.

## Пользовательские инварианты
<!-- Добавляйте свои инварианты ниже в формате: - Текст инварианта -->

_Нет пользовательских инвариантов_

## Автоматически извлечённые
<!-- Эти инварианты извлекаются автоматически из диалога -->

_Нет автоматических инвариантов_
""".trimIndent()

        try {
            invariantsFile.writeText(defaultContent)
            println("┃  📌 Created invariants file: ${invariantsFile.absolutePath}")
        } catch (e: Exception) {
            println("Error creating invariants file: ${e.message}")
        }
    }
    ...
    fun buildMemoryContext(chatId: String): String {
        ...
        return buildString {
            ...
            // Invariants - rules that must always be followed
            if (invariants.isNotEmpty()) {
                appendLine("=== ИНВАРИАНТЫ (всегда соблюдать) ===")
                invariants.forEach { appendLine("- ${it.text}") }
                appendLine()
            }
